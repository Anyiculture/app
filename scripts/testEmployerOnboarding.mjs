...
// Run: node scripts/testEmployerOnboarding.mjs
...
async function createTestEmployer() {
  ...
  const employerData = {
    ...
    registration_province: 'California',
  ...
  console.log('Job Posted:    ', jobPostData?.[0]?.title || 'Failed');
  console.log('='.repeat(60));
...
createTestEmployer()
  .then((result) => {
    if (result) {
      ...
      console.log('\n💡 You can now sign in with these credentials to test the UI');
    }
    process.exit(0);
  })
  .catch((error) => {
