-- =====================================================
-- Repair missing get_or_create_direct_conversation RPC
-- Date: 2026-03-13
-- =====================================================
-- Purpose:
-- 1) Recreate the unified conversation creation RPC if it is missing.
-- 2) Recreate the helper functions it depends on.
-- 3) Reload PostgREST schema cache so Supabase sees the RPC immediately.
-- =====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Missing public.normalize_host_family_subscription_status(). Apply the host-family subscription migration first.';
  END IF;
END $$;
